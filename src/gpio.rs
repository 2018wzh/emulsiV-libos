...
pub struct Gpio<'a, B: RegisterIo> {
    bus: &'a mut B,
}
impl<'a, B: RegisterIo> Gpio<'a, B> {
    ...
    pub fn direction(&mut self) -> u32 {
        self.bus.read32(DIR)
    }
    /// Direction bit 1 is input; bit 0 is output.
    pub fn set_direction(&mut self, inputs: u32) {
        self.bus.write32(DIR, inputs);
    }
    pub fn configure_inputs(&mut self, mask: u32) {
        let v = self.direction();
        self.set_direction(v | mask);
    }
    ...
    pub fn read_pin(&mut self, pin: Pin) -> bool {
        self.read() & pin.mask() != 0
    }
    pub fn write(&mut self, value: u32) {
        self.bus.write32(VALUE, value);
    }
    ...
    pub fn write_masked(&mut self, mask: u32, value: u32) {
        ...
        let old = self.read();
        self.write((old & !outputs) | (value & outputs));
    }
    pub fn set_high(&mut self, mask: u32) {
        self.write_masked(mask, mask);
    }
    pub fn set_low(&mut self, mask: u32) {
        self.write_masked(mask, 0);
    }
    pub fn toggle(&mut self, mask: u32) {
        let v = self.read();
        self.write_masked(mask, !v);
    }
    pub fn set_interrupt_mask(&mut self, mask: u32) {
        self.bus.write32(IEN, mask);
    }
    pub fn edges(&mut self) -> Edges {
        Edges {
            rising: self.bus.read32(RISING),
            falling: self.bus.read32(FALLING),
        }
    }
    ...
    pub fn clear_edges(&mut self, mask: u32) {
        let r = self.bus.read32(RISING);
        self.bus.write32(RISING, r & !mask);
        let f = self.bus.read32(FALLING);
        self.bus.write32(FALLING, f & !mask);
    }
    pub fn clear_all_edges(&mut self) {
        self.bus.write32(RISING, 0);
        self.bus.write32(FALLING, 0);
    }
}
