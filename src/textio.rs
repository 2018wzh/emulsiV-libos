...
pub struct TextIo<'a, B: RegisterIo> { bus: &'a mut B }
impl<'a, B: RegisterIo> TextIo<'a, B> {
    pub fn new(bus: &'a mut B) -> Self { Self { bus } }
    pub fn put_byte(&mut self, byte: u8) { self.bus.write8(TEXT_OUT, byte); }
    pub fn write_bytes(&mut self, bytes: &[u8]) { for &b in bytes { self.put_byte(b); } }
    pub fn write_str(&mut self, s: &str) { self.write_bytes(s.as_bytes()); }
    pub fn available(&mut self) -> bool { self.bus.read8(TEXT_CTRL) & RECEIVED != 0 }
    pub fn try_read(&mut self) -> Option<u8> {
        let status = self.bus.read8(TEXT_CTRL);
        if status & RECEIVED == 0 { return None; }
    ...
    pub fn write_u32(&mut self, mut value: u32) {
        ...
        loop {
            i -= 1; digits[i] = b'0' + (value % 10) as u8; value /= 10;
            if value == 0 { break; }
        }
        self.write_bytes(&digits[i..]);
    }
    pub fn write_i32(&mut self, value: i32) {
        if value < 0 { self.put_byte(b'-'); }
