...
impl<const N: usize> Scheduler<N> {
    ...
    pub fn add(
        ...
        if period == 0 || period > MAX_INTERVAL {
    ...
    pub fn cancel(&mut self, id: TaskId) -> bool {
        if let Some(slot) = self.tasks.get_mut(id.slot) {
            if slot.as_ref().map(|t| t.generation) == Some(id.generation) {
                *slot = None;
                return true;
            }
        }
        ...
        for offset in 0..N {
            ...
            if let Some(mut task) = self.tasks[i] {
                if reached(now, task.next) {
                    let result = (task.run)(now);
                    calls += 1;
                    if result == TaskControl::Finished {
                        self.tasks[i] = None;
                    } else {
                        task.next = now.wrapping_add(task.period);
                        self.tasks[i] = Some(task);
                    }
                    self.cursor = (i + 1) % N;
                    if calls == budget {
