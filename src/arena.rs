...
pub struct Arena<'a> { rest:&'a mut [u8],used:usize }
impl<'a> Arena<'a> {
    pub fn new(storage:&'a mut [u8])->Self {Self {rest:storage,used:0}}
    pub fn used(&self)->usize {self.used}
    pub fn remaining(&self)->usize {self.rest.len()}
