...
pub struct Bitmap<'a, B: RegisterIo> { bus: &'a mut B }
impl<'a, B: RegisterIo> Bitmap<'a, B> {
    pub fn new(bus: &'a mut B) -> Self { Self { bus } }
    pub fn clear(&mut self, color: Color) {
        for offset in 0..1024 { self.bus.write8(FRAMEBUFFER + offset, color.bits()); }
    }
    pub fn get_pixel(&mut self, x: i16, y: i16) -> Option<Color> {
        if !(0..32).contains(&x) || !(0..32).contains(&y) { return None; }
        Some(Color::from_bits(self.bus.read8(FRAMEBUFFER + y as usize * 32 + x as usize)))
    }
    ...
    pub fn scroll_up(&mut self, rows: u8, background: Color) {
        ...
        for i in 1024-shift..1024 { self.bus.write8(FRAMEBUFFER+i, background.bits()); }
    }
}
impl<B: RegisterIo> PixelTarget for Bitmap<'_, B> {
    fn size(&self) -> (u16, u16) { (WIDTH, HEIGHT) }
    fn pixel(&mut self, x: i16, y: i16, c: Color) {
        if (0..32).contains(&x) && (0..32).contains(&y) {
            self.bus.write8(FRAMEBUFFER + y as usize * 32 + x as usize, c.bits());
        }
    }
}
fn plot<T: PixelTarget>(t: &mut T, x: i32, y: i32, c: Color) {
    ...
    if x >= 0 && y >= 0 && x < i32::from(w) && y < i32::from(h)
        && x <= i32::from(i16::MAX) && y <= i32::from(i16::MAX) {
...
pub fn fill_rect<T: PixelTarget>(t: &mut T, x: i16, y: i16, w: u16, h: u16, c: Color) {
    ...
    for yy in i32::from(y).max(0)..bottom {
        for xx in i32::from(x).max(0)..right { t.pixel(xx as i16,yy as i16,c); }
    }
}
pub fn rect<T: PixelTarget>(t: &mut T, x: i16, y: i16, w: u16, h: u16, c: Color) {
    if w == 0 || h == 0 { return; }
    fill_rect(t,x,y,w,1,c); fill_rect(t,x,y,1,h,c);
    let bottom = i32::from(y)+i32::from(h)-1;
    let right = i32::from(x)+i32::from(w)-1;
    if bottom <= i32::from(i16::MAX) { fill_rect(t,x,bottom as i16,w,1,c); }
    if right <= i32::from(i16::MAX) { fill_rect(t,right as i16,y,1,h,c); }
}
...
fn glyph(ch: u8) -> u16 {
    match ch.to_ascii_uppercase() {
        b'0' => 0b111_101_101_101_111, b'1' => 0b010_110_010_010_111,
        b'2' => 0b110_001_010_100_111, b'3' => 0b110_001_010_001_110,
        b'4' => 0b101_101_111_001_001, b'5' => 0b111_100_110_001_110,
        b'6' => 0b011_100_111_101_111, b'7' => 0b111_001_010_010_010,
        b'8' => 0b111_101_111_101_111, b'9' => 0b111_101_111_001_110,
        b'A' => 0b010_101_111_101_101, b'B' => 0b110_101_110_101_110,
        b'C' => 0b011_100_100_100_011, b'D' => 0b110_101_101_101_110,
        b'E' => 0b111_100_110_100_111, b'F' => 0b111_100_110_100_100,
        b'G' => 0b011_100_101_101_011, b'H' => 0b101_101_111_101_101,
        b'I' => 0b111_010_010_010_111, b'J' => 0b001_001_001_101_010,
        b'K' => 0b101_101_110_101_101, b'L' => 0b100_100_100_100_111,
        b'M' => 0b101_111_111_101_101, b'N' => 0b101_111_111_111_101,
        b'O' => 0b010_101_101_101_010, b'P' => 0b110_101_110_100_100,
        b'Q' => 0b010_101_101_111_011, b'R' => 0b110_101_110_101_101,
        b'S' => 0b011_100_010_001_110, b'T' => 0b111_010_010_010_010,
        b'U' => 0b101_101_101_101_111, b'V' => 0b101_101_101_101_010,
        b'W' => 0b101_101_111_111_101, b'X' => 0b101_101_010_101_101,
        b'Y' => 0b101_101_010_010_010, b'Z' => 0b111_001_010_100_111,
        b' ' => 0, b'-' => 0b000_000_111_000_000, b'_' => 0b000_000_000_000_111,
        b'.' => 1, b':' => 0b000_010_000_010_000, b'!' => 0b010_010_010_000_010,
...
pub fn text<T: PixelTarget>(t: &mut T, x: i16, y: i16, s: &str, c: Color) {
    ...
    for ch in s.bytes() {
        if ch == b'\n' { xx=x; yy=yy.saturating_add(6); }
        else { draw_char(t,xx,yy,ch,c); xx=xx.saturating_add(4); }
    }
}
...
pub fn blit_mono<T: PixelTarget>(t: &mut T, x: i16, y: i16, w: u8, h: u8,
    data: &[u8], foreground: Color, background: Option<Color>) -> Result<(),ShortSprite> {
    let stride = usize::from(w).div_ceil(8);
    if data.len() < stride * usize::from(h) { return Err(ShortSprite); }
...
pub struct MonoBuffer { bits: [u8;128] }
impl Default for MonoBuffer { fn default() -> Self { Self::new() } }
impl MonoBuffer {
    pub const fn new() -> Self { Self { bits: [0;128] } }
    pub fn clear(&mut self) { self.bits.fill(0); }
    pub fn bytes(&self) -> &[u8;128] { &self.bits }
...
impl PixelTarget for MonoBuffer {
    fn size(&self) -> (u16,u16) { (32,32) }
    fn pixel(&mut self, x: i16, y: i16, c: Color) {
        if !(0..32).contains(&x) || !(0..32).contains(&y) { return; }
        let index = y as usize*32+x as usize; let mask = 0x80 >> (index%8);
        if c == Color::BLACK { self.bits[index/8] &= !mask; } else { self.bits[index/8] |= mask; }
    }
}
