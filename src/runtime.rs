...
use core::{arch::global_asm, panic::PanicInfo, sync::atomic::{compiler_fence, Ordering}};
use crate::{bus::{Mmio, RegisterIo, TEXT_CTRL}, gpio::IEN};
...
pub fn halt() -> ! { loop { unsafe { core::arch::asm!("j .", options(nomem, nostack)) } } }
...
pub struct MemoryInfo {pub image_end:usize,pub stack_bottom:usize,pub stack_top:usize}
