...
pub enum LineEvent { None, Echo(u8), Erase, Cleared, Complete, Overflow, Cancelled }
pub struct LineEditor<const N:usize> { data:[u8;N],len:usize,overflow:bool,ready:bool,after_cr:bool }
impl<const N:usize> Default for LineEditor<N> { fn default()->Self { Self::new() } }
impl<const N:usize> LineEditor<N> {
    ...
    pub fn feed(&mut self,b:u8)->LineEvent {
        if b==b'\n' && self.after_cr { self.after_cr=false; return LineEvent::None; }
        self.after_cr=b==b'\r';
        if self.ready { self.clear(); }
        match b {
            b'\r'|b'\n' => {
                self.ready=true;
                if self.overflow { LineEvent::Overflow } else { LineEvent::Complete }
            }
            3 => { self.clear(); LineEvent::Cancelled }
            21 => { self.clear(); LineEvent::Cleared }
            8|127 => {
