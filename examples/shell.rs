...
use os::{
    bitmap::{Bitmap, Color, PixelTarget},
    console::{LineEditor, LineEvent},
    shell::Command,
};
fn execute(bus: &mut Mmio, line: &str) {
    match os::shell::parse(line) {
        ...
        Ok(Command::ReadGpio) => {
            ...
            let mut io = os::textio::TextIo::new(bus);
            io.write_hex32(value);
            io.put_byte(b'\n');
        }
        ...
        Ok(Command::Pixel { x, y, color }) => {
            Bitmap::new(bus).pixel(i16::from(x), i16::from(y), Color::from_bits(color))
        }
